...
function findHeaderRow(sheet: XLSX.WorkSheet): number {
  ...
  const claves = new Set([
    'id', 'fecha', 'total', 'estado', 'fiscal', 'caja', 'venta', 'medio',
    'nombre', 'categoria', 'subcategoria', 'cantidad', 'codigo',
    'monto', 'cancelado', 'cliente', 'pago',
  ])
  ...
  for (let r = 0; r <= Math.min(7, range.e.r); r++) {
    ...
    for (let c = 0; c <= Math.min(15, range.e.c); c++) {
      ...
      if (claves.has(v) || claves.has(v.split(' ')[0]) || claves.has(v.split('.')[0])) coincidencias++
    }
...
export function parseFudoVentas(buffer: ArrayBuffer, local: 'vedia' | 'saavedra'): ParsedVentas {
  ...
  // ---------- Hoja Descuentos ----------
  let descuentos: DescuentoResumen = { cortesias_cant: 0, cortesias_monto: 0, otros_descuentos_monto: 0 }

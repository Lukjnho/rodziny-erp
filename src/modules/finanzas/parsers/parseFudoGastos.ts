...
export function parseFudoGastos(
  ...
  try {
    ...
    rawImp = XLSX.utils.sheet_to_json<Record<string, unknown>>(wsImp, {
      range: headerRowImp,
      defval: 0,
    });
    if (rawImp.length > 0) {
  ...
  console.log(
    '[gastos] impMap size:',
    impMap.size,
    impMap.size > 0 ? '| primer entry:' : '',
    impMap.size > 0 ? [...impMap.entries()][0] : '',
  );
  ...
  const gastos: GastoRow[] = rawGastos
    ...
    .map((r) => {
      ...
      const importeSinIva = parseNum(
        r['Importe SIN IVA'] ?? r['Importe sin IVA'] ?? r['Importe neto'] ?? 0,
      );
      return {
        fudo_id: id,
        ...
        importe_neto:
          importeSinIva > 0 && Math.abs(importeSinIva - importeTotal) > 1
            ? importeSinIva
            : imp
              ? imp.neto
              : null,
        ...
        de_caja:
          String(r['De Caja'] ?? '').toLowerCase() === 'sí' ||
          String(r['De Caja'] ?? '').toLowerCase() === 'si',
        cancelado:
          String(r['Cancelado'] ?? '').toLowerCase() === 'sí' ||
          String(r['Cancelado'] ?? '').toLowerCase() === 'si',
      };
    });
  ...
  try {
    ...
    const rawDetalle = XLSX.utils.sheet_to_json<Record<string, unknown>>(wsDetalle, {
      range: headerRowDet,
      defval: '',
    });

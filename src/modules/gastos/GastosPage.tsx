...
export function GastosPage() {
  ...
  // ── construir árbol categoria → subcategoria → por mes ───────────────────
  const { categorias, mesesConDatos } = useMemo(() => {
    ...
    for (const g of rawGastos ?? []) {
      ...
      const mes = g.periodo
      // usar importe_neto (sin IVA) si existe, sino importe_total
  ...
  // Meses a mostrar: todos los del año (los sin datos muestran —)
  const meses = useMemo(
  ...
  // ── render ────────────────────────────────────────────────────────────────
  return (
    <PageContainer title="Gastos" subtitle="Egresos por categoría · datos desde Fudo">
      {/* Filtros */}
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center h-40 text-gray-400 text-sm gap-2">
          ...
          <span className="text-xs">Importá el archivo de Gastos Fudo desde Finanzas → Importar datos.</span>
        </div>
      ) : (
                  ...
                  return [
                    ...
                    ...(isOpen
                      ? Array.from(cat.subcats.values()).map((sub) => {
                          ...
                          // Ocultar subcategoría si tiene el mismo nombre que la categoría (sin subcategoría real)

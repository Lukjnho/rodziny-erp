...
export function PedidosTab() {
  ...
  const cambiarEstado = useMutation({
    mutationFn: async ({ id, estado }: { id: string; estado: Estado }) => {
      const { error } = await supabase.from('almacen_pedidos').update({ estado }).eq('id', id);
      if (error) throw error;
    },
  ...
  const grupoManana = pedidosFiltrados.filter(
    (p) => p.fecha_entrega === manana() && p.estado !== 'entregado' && p.estado !== 'cancelado',
  );
  const grupoProximos = pedidosFiltrados.filter(
    (p) => p.fecha_entrega > manana() && p.estado !== 'entregado' && p.estado !== 'cancelado',
  );
  const grupoCompletados = pedidosFiltrados.filter(
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-4 gap-3">
        <KpiCard
          label="Para hoy"
          valor={pendientesHoy}
          color={pendientesHoy > 0 ? 'text-red-600' : 'text-gray-400'}
          bg={pendientesHoy > 0 ? 'bg-red-50 border-red-200' : 'bg-gray-50 border-gray-200'}
        />
        <KpiCard
          label="Para mañana"
          valor={pendientesManana}
          color={pendientesManana > 0 ? 'text-amber-600' : 'text-gray-400'}
          bg={pendientesManana > 0 ? 'bg-amber-50 border-amber-200' : 'bg-gray-50 border-gray-200'}
        />
        <KpiCard
      ...
      {isLoading ? (
        ...
        <div className="rounded-lg border border-gray-200 bg-white p-8 text-center">
          ...
          <p className="text-sm text-gray-500">
            No hay pedidos{' '}
            {filtroEstado !== 'todos'
              ? `con estado "${ESTADOS.find((e) => e.valor === filtroEstado)?.label}"`
              : 'cargados'}
          </p>
        </div>
      ) : (
        <div className="space-y-4">
          ...
          {grupoProximos.length > 0 && (
            <GrupoPedidos
              titulo="Próximos"
              badge={grupoProximos.length}
              color="blue"
              pedidos={grupoProximos}
              onCambiarEstado={(id, est) => cambiarEstado.mutate({ id, estado: est })}
              onEditar={(p) => {
                setEditando(p);
                setModalAbierto(true);
          ...
          {grupoCompletados.length > 0 && (
            <GrupoPedidos
              titulo="Completados"
              badge={grupoCompletados.length}
              color="gray"
              pedidos={grupoCompletados}
              onCambiarEstado={(id, est) => cambiarEstado.mutate({ id, estado: est })}
              onEditar={(p) => {
                setEditando(p);
                setModalAbierto(true);
      ...
      {modalAbierto && (
        <ModalPedido
          ...
          onGuardado={() => {
            setModalAbierto(false);
            setEditando(null);
            qc.invalidateQueries({ queryKey: ['almacen-pedidos'] });
          }}
        />
      )}
    </div>
  ...
  return (
    <div className="overflow-hidden rounded-lg border border-gray-200 bg-white">
      <div className="flex items-center gap-2 border-b border-gray-100 px-4 py-2.5">
        ...
        <span className={cn('rounded-full px-2 py-0.5 text-xs font-medium', colorMap[color])}>
          {badge}
        </span>
      </div>
    ...
    <div className="flex items-center gap-4 px-4 py-3 transition-colors hover:bg-gray-50">
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="flex items-center gap-3 text-xs text-gray-500">
          ...
          <span>
            Entrega: {formatFecha(pedido.fecha_entrega)} ({pedido.turno})
          </span>
          {pedido.vendedor && <span>Vendedor: {pedido.vendedor}</span>}
        </div>
        {pedido.observaciones && (
          <div className="mt-0.5 text-xs italic text-gray-400">{pedido.observaciones}</div>
        )}
      </div>

      {/* Pago */}
      <div className="flex-shrink-0 space-y-0.5 text-right text-xs">
        {pedido.abono && <div className="font-medium text-green-600">Abonado</div>}
      ...
      <div className="flex flex-shrink-0 items-center gap-1">
        {siguiente && (
          <button
            ...
            className="rounded bg-rodziny-50 px-2.5 py-1 text-xs text-rodziny-700 transition-colors hover:bg-rodziny-100"
          >
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="relative">
              ...
              {sugerenciasCliente && clientesFiltrados.length > 0 && (
                <div className="absolute top-full z-10 mt-1 max-h-32 w-full overflow-y-auto rounded-md border border-gray-200 bg-white shadow-md">
                  {clientesFiltrados.map((c) => (
                    ...
                    >
                      {c.nombre}{' '}
                      {c.telefono && (
                        <span className="ml-1 text-xs text-gray-400">{c.telefono}</span>
                      )}
                    </button>
                  ))}
                </div>

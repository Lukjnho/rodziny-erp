  ...
  const { data: pedidos, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .select('id, producto_nombre, cantidad, cliente_nombre, cliente_telefono, fecha_entrega, turno, estado, observaciones')
    ...
    <div className="space-y-4">
      ...
      {resumenOrdenado.length > 0 && (
        <div className="bg-white rounded-lg border border-gray-200 p-3">
          <div className="text-xs font-medium text-gray-500 mb-2">Resumen de la semana — {pedidos?.filter(p => p.estado !== 'entregado').length ?? 0} pedidos pendientes</div>
          <div className="flex flex-wrap gap-2">
            {resumenOrdenado.map(([nombre, qty]) => (
              <span key={nombre} className="text-xs bg-rodziny-50 text-rodziny-700 px-2 py-1 rounded-full font-medium">
                {qty}x {nombre}
              </span>
            ))}
          </div>
            ...
            return (
              ...
              >
                ...
                <div className={cn(
                  ...
                  <div className={cn('text-xs font-semibold', esHoy ? 'text-rodziny-700' : 'text-gray-600')}>
                    {DIAS_SEMANA[i]}
                  </div>
                  <div className={cn('text-lg font-bold', esHoy ? 'text-rodziny-800' : 'text-gray-800')}>
                    {dia.getDate()}
                  </div>
                  {pedidosDia.length > 0 && (
                    <div className="text-[10px] text-gray-400">{pedidosDia.length} pedido{pedidosDia.length > 1 ? 's' : ''}</div>
                  )}
                </div>
                ...
                <div className="flex-1 p-1.5 space-y-1 overflow-y-auto">
                  {pedidosDia.length === 0 && (
                    <div className="text-[10px] text-gray-300 text-center mt-4">Sin pedidos</div>
                  )}
                  {pedidosDia.map((p) => {
                      ...
                      >
                        <div className="flex items-center gap-1">
                          ...
                          <span className={cn('text-[11px] font-medium truncate', colors.text)}>
                            {p.cantidad > 1 ? `${p.cantidad}x ` : ''}{p.producto_nombre}
                          </span>
                        </div>
                        <div className="text-[10px] text-gray-400 truncate pl-2.5">
      ...
      <div className="flex items-center justify-center gap-4 text-[10px] text-gray-400">
        {Object.entries(ESTADO_COLORES).filter(([k]) => k !== 'cancelado').map(([key, val]) => (
          <div key={key} className="flex items-center gap-1">
            <span className={cn('w-2 h-2 rounded-full', val.dot)} />
            <span className="capitalize">{key === 'en_preparacion' ? 'En preparación' : key}</span>
          </div>
        ))}
      </div>
    </div>

...
function calcularScore(empleadoId: string, cronograma: Cronograma[], fichadas: Fichada[]): number {
  ...
  const crono = cronograma.filter(
    (c) => c.empleado_id === empleadoId && c.fecha >= desde && c.fecha <= hasta && c.publicado && !c.es_franco && c.hora_entrada,
  )
  if (crono.length < 5) return 0

  const fichSet = new Set(fichadas.filter((f) => f.empleado_id === empleadoId && f.tipo === 'entrada').map((f) => f.fecha))
  ...
  for (const c of crono) {
    ...
    if (entrada && entrada.minutos_diferencia !== null && entrada.minutos_diferencia > 10) tardanzas++
    else puntuales++
  }
  ...
  const cambiarEstado = useMutation({
    mutationFn: async ({ id, estado }: { id: string; estado: EstadoVac }) => {
      const patch: Partial<Vacacion> = {
        estado,
        aprobado_por: estado === 'aprobada' || estado === 'rechazada' ? perfil?.nombre || null : null,
      }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
        ...
        <KPICard label="Días a otorgar" value={String(kpis.diasAOtorgar)} color="neutral" loading={cargando} />
        <KPICard label="Días tomados" value={String(kpis.diasTomados)} color="green" loading={cargando} />
        <KPICard label="Pendientes aprobación" value={String(kpis.pendientes)} color={kpis.pendientes > 0 ? 'yellow' : 'neutral'} loading={cargando} />
        <KPICard label="Sin tomar aún" value={String(kpis.sinTomar)} color={kpis.sinTomar > 0 ? 'yellow' : 'green'} loading={cargando} />
      </div>
      ...
      {pendientes.length > 0 && (
        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3">
          <div className="text-sm font-semibold text-yellow-900 mb-2">⏳ Solicitudes pendientes de aprobación ({pendientes.length})</div>
          <div className="space-y-2">
            {pendientes.map((v) => {
              ...
              return (
                <div key={v.id} className="bg-white rounded px-3 py-2 flex items-center gap-3 text-sm">
                  <div className="flex-1">
                    <span className="font-medium text-gray-900">{emp.apellido}, {emp.nombre}</span>
                    <span className="text-gray-500 ml-2">
                      {formatearRango(v.fecha_desde, v.fecha_hasta)} · {v.dias_corridos} días
                    </span>
                    {fueraTemporada && <span className="ml-2 text-[10px] bg-orange-100 text-orange-700 px-1.5 py-0.5 rounded">fuera de temporada</span>}
                    {v.motivo && <div className="text-xs text-gray-400 mt-0.5">{v.motivo}</div>}
                  </div>
                  <button
        ...
        <table className="w-full text-sm">
          <thead className="bg-gray-50 text-xs uppercase text-gray-500">
            <tr>
              ...
              <th className="text-center px-2 py-2">Antigüedad</th>
              <th className="text-center px-2 py-2">Corresponden</th>
              <th className="text-center px-2 py-2">Tomados</th>
              <th className="text-center px-2 py-2">Pendientes</th>
          ...
          <tbody>
            ...
            {!cargando && filas.length === 0 && (
              <tr><td colSpan={8} className="text-center py-8 text-gray-400">Sin empleados</td></tr>
            )}
            {filas.map((f, idx) => (
              <tr key={f.empleado.id} className={cn('border-t border-gray-100 hover:bg-gray-50', !f.elegible && 'opacity-50')}>
                <td className="px-4 py-2">
                  <div className="font-medium text-gray-900">
                    {idx < 3 && f.score >= 80 && <span className="mr-1">{['🥇','🥈','🥉'][idx]}</span>}
                ...
                <td className="px-2 py-2 text-center">
                  {f.score > 0 ? (
                    <span className={cn(
                      ...
                      f.score >= 90 ? 'bg-green-100 text-green-700' :
                      f.score >= 75 ? 'bg-blue-100 text-blue-700' :
                      f.score >= 60 ? 'bg-yellow-100 text-yellow-700' :
                ...
                <td className="px-2 py-2 text-center text-xs text-gray-600">
                  {f.antAños >= 1 ? `${Math.floor(f.antAños)} año${Math.floor(f.antAños) !== 1 ? 's' : ''}` :
                   `${Math.floor(f.antAños * 12)} meses`}
                </td>
                <td className="px-2 py-2 text-center text-gray-700 font-medium">{f.diasCorresponden}</td>
                ...
                <td className="px-2 py-2 text-center">
                  <span className={cn(
                    'font-semibold',
                    f.saldo === 0 ? 'text-gray-400' : f.saldo <= 7 ? 'text-yellow-600' : 'text-green-600',
                  )}>
                    {f.saldo}
                  </span>
                </td>
                <td className="px-4 py-2 text-right">
                  {f.elegible ? (
                    <button
                      ...
                      className="text-xs px-2 py-1 bg-rodziny-600 text-white rounded hover:bg-rodziny-700"
                    >
                      + Vacaciones
                    </button>
                  ) : (
              ...
              {vacaciones.map((v) => {
                ...
                return (
                  <tr key={v.id} className="border-t border-gray-100 hover:bg-gray-50">
                    <td className="px-4 py-2 text-gray-900">{emp.apellido}, {emp.nombre}</td>
                    <td className="px-2 py-2 text-gray-600">{formatearRango(v.fecha_desde, v.fecha_hasta)}</td>
                    <td className="px-2 py-2 text-center">{v.dias_corridos}</td>
                    <td className="px-2 py-2 text-center"><BadgeEstado estado={v.estado} /></td>
                    ...
                    <td className="px-4 py-2 text-right space-x-1">
                      ...
                      <button
                        onClick={() => {
                          ...
                          if (emp2) { setModalEmpleado(emp2); setEditVacacion(v) }
                        }}
      ...
      {modalEmpleado && (
        <ModalVacacion
          ...
          onClose={() => { setModalEmpleado(null); setEditVacacion(null) }}
          onSaved={() => {
            qc.invalidateQueries({ queryKey: ['vacaciones'] })
            setModalEmpleado(null); setEditVacacion(null)
          }}
        />
      )}
    </div>
...
function ModalVacacion({
  empleado, año, vacacionEdit, onClose, onSaved,
}: {
  empleado: Empleado
  año: number
  ...
  const fueraTemporada = fechaDesde && fechaHasta ? !estaEnTemporadaBaja(fechaDesde, fechaHasta) : false

  async function guardar() {
    setError(null)
    if (!fechaDesde) { setError('Elegí la fecha de inicio.'); return }
    if (dias < 1) { setError('Mínimo 1 día.'); return }
    setGuardando(true)
    try {
      const payload = {
      ...
      if (vacacionEdit) {
        const { error } = await supabase.from('vacaciones').update(payload).eq('id', vacacionEdit.id)
        if (error) throw error
      } else {
    ...
    <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-md" onClick={(e) => e.stopPropagation()}>
        <div className="px-6 py-4 border-b border-gray-100">
          ...
          <div className="text-xs text-gray-500 mt-0.5">{empleado.apellido}, {empleado.nombre}</div>
        </div>
        <div className="px-6 py-4 space-y-3">
          ...
          <div>
            <label className="block text-xs font-medium text-gray-600 mb-1">Días corridos *</label>
            <input
              type="number"
              min={1}
              max={35}
          ...
          <div>
            <label className="block text-xs font-medium text-gray-600 mb-1">Motivo / Comentario</label>
            <input
              value={motivo}
...
function BadgeEstado({ estado }: { estado: EstadoVac }) {
  ...
  return (
    <span className={`text-[10px] px-1.5 py-0.5 rounded font-medium ${config.bg} ${config.fg}`}>
      {config.label}
    </span>
  )
}

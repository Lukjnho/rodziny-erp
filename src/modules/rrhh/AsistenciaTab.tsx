  ...
  const resumenPorDia: DiaResumen[] = useMemo(() => {
    ...
    return dias.map((fecha) => {
      const fichadasDelDia = fichadas.filter(
        (f) => f.fecha === fecha && empleadoIdsFiltrados.has(f.empleado_id),
      );
      const cronoDelDia = cronograma.filter(
        (c) => c.fecha === fecha && empleadoIdsFiltrados.has(c.empleado_id),
      );
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-3 rounded-lg border border-gray-200 bg-white p-3">
        <div className="flex items-center gap-1">
          ...
          <span className="min-w-[160px] text-center text-sm font-medium text-gray-900">
            {MESES[month]} {year} ·{' '}
            {quincena === 'q1' ? 'Q1 (1-14)' : `Q2 (15-${ultimoDiaDelMes(year, month)})`}
          </span>
          <button
            onClick={() => navegarQuincena(1)}
            className="rounded px-2 py-1 text-gray-500 hover:bg-gray-100"
        ...
        <div className="ml-auto flex items-center gap-1">
          <button
            ...
            className="rounded bg-rodziny-700 px-3 py-1.5 text-xs font-medium text-white hover:bg-rodziny-800"
          >
            + Fichaje manual
          </button>
          <div ref={menuRef} className="relative">
            <button
              ...
              className="rounded px-2 py-1.5 text-base leading-none text-gray-500 hover:bg-gray-100"
              title="Más opciones"
            >
              ⋯
            </button>
            {menuAbierto && (
              <div className="absolute right-0 top-full z-10 mt-1 w-56 rounded-md border border-gray-200 bg-white py-1 shadow-lg">
                <button
                  ...
                  disabled={limpiando}
                  className="w-full px-3 py-2 text-left text-xs text-gray-700 hover:bg-gray-50 disabled:opacity-50"
                >
      ...
      <div className="divide-y divide-gray-100 rounded-lg border border-gray-200 bg-white">
        {[...resumenPorDia].reverse().map((d) => {
          ...
          const tieneActividad =
            d.fichadasDelDia.length > 0 || d.ausencias > 0 || d.incompletos > 0 || d.enTurno > 0;
          ...
          return (
            <div key={d.fecha}>
              ...
              >
                <span className="w-28 text-sm font-medium capitalize text-gray-900">
                  {etiquetaDia(d.fecha)}
                </span>
                ...
                >
                  {d.completos > 0 && (
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        setFiltroEstado(filtroEstado === 'completas' ? 'todos' : 'completas');
                        setDiaExpandido(d.fecha);
                      }}
                      className={cn(
                        'text-green-700 hover:underline',
                        filtroEstado === 'completas' && 'font-bold underline',
                      )}
                    >
                      ✓ {d.completos} completos
                    </button>
                  )}
                  {d.enTurno > 0 && <span className="text-blue-700">{d.enTurno} en turno</span>}
                  {d.incompletos > 0 && (
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        setFiltroEstado(filtroEstado === 'incompletas' ? 'todos' : 'incompletas');
                        setDiaExpandido(d.fecha);
                      }}
                      className={cn(
                        'text-amber-700 hover:underline',
                        filtroEstado === 'incompletas' && 'font-bold underline',
                      )}
                    >
                      ⚠ {d.incompletos} incompletos
                    </button>
                  )}
                  {d.ausencias > 0 && (
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        setFiltroEstado(filtroEstado === 'ausencias' ? 'todos' : 'ausencias');
                        setDiaExpandido(d.fecha);
                      }}
                      className={cn(
                        'text-red-700 hover:underline',
                        filtroEstado === 'ausencias' && 'font-bold underline',
                      )}
                    >
                      ✗ {d.ausencias} ausentes
                    </button>
                  )}
                  {d.tardanzas > 0 && (
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        setFiltroEstado(filtroEstado === 'tardanzas' ? 'todos' : 'tardanzas');
                        setDiaExpandido(d.fecha);
                      }}
                      className={cn(
                        'text-amber-700 hover:underline',
                        filtroEstado === 'tardanzas' && 'font-bold underline',
                      )}
                    >
                      ⏱ {d.tardanzas} tarde
                    </button>
                  ...
                  {!tieneActividad && fechaPasada && (
                    <span className="text-gray-400">Sin actividad</span>
                  )}
                  {!fechaPasada && !tieneActividad && (
                    <span className="text-gray-400">A futuro</span>
                  )}
                </div>

                <span className="text-sm text-gray-400">{expandido ? '▾' : '▸'}</span>
              </button>
...
function DetalleDia({
  ...
  cronograma.forEach((c) => {
    if (c.publicado || c.es_franco) idsConActividad.add(c.empleado_id);
  });
  ...
  function renderCard({ emp, estado, tarde }: (typeof filasConEstado)[number]) {
    ...
    return (
      <div
        key={emp.id}
        className={cn(
          'rounded border bg-white p-3 transition-opacity',
          esAusente && 'border-red-200 bg-red-50',
      ...
      >
        <div className="mb-2 flex items-start justify-between">
          ...
          <button
            ...
            className="text-[11px] text-rodziny-700 underline hover:text-rodziny-800"
          >
            + Fichada
          </button>
        </div>
        ...
        {fs.length === 0 ? (
          <p className="text-[11px] italic text-gray-400">Sin fichajes</p>
        ) : (
      ...
      {mostrarSeparadores ? (
        <>
          ...
          {grupoSaavedra.length > 0 && (
            <>
              <p className="pt-2 text-[10px] font-semibold uppercase tracking-wide text-gray-400">
                Saavedra
              </p>
...
function FilaFichada({
  ...
  async function cargarFoto() {
    if (!fichada.foto_path || fotoUrl) {
      setVerFoto(true);
      return;
    }
    const { data, error } = await supabase.storage
      .from('fichadas-fotos')
      .createSignedUrl(fichada.foto_path, 300);
    ...
    <>
      <div className="flex items-center gap-2 text-xs">
        {esSalidaNocturnaLegacy(fichada) ? (
          <span
            className="w-14 font-medium text-purple-600"
            title="Salida del turno nocturno del día anterior"
          >
            Salida
          </span>
        ) : (
          <span className="w-14 capitalize text-gray-700">{fichada.tipo}</span>
        )}
        <span className="w-12 font-mono text-gray-900">{hora}</span>
        {esSalidaNocturnaLegacy(fichada) ? (
          <span className="w-20 text-[10px] text-purple-500">turno ant.</span>
        ) : dif !== null ? (
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          <button onClick={eliminar} className="text-red-600 hover:text-red-700">
            ×
          </button>
        </div>
      </div>
      ...
      {verFoto && fotoUrl && (
        ...
        >
          <img src={fotoUrl} alt="fichada" className="max-h-full max-w-full rounded" />
        </div>
      )}
    </>
  );
      ...
      >
        ...
        {empleado && (
          <p className="mb-3 text-xs text-gray-500">
            {empleado.nombre} {empleado.apellido} · {fichada.fecha}
          </p>
        )}
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      ...
      >
        ...
        <p className="mb-3 text-xs text-gray-500">
          Cargá un fichaje manualmente cuando alguien se olvidó el celular o no pudo fichar por la
          app.
        </p>
        ...
        {!empleadoIdInicial && (
          <>
            ...
            <input
              ...
              value={busqueda}
              onChange={(e) => setBusqueda(e.target.value)}
              className="mb-1 w-full rounded border border-gray-300 px-2 py-1.5 text-sm"
            />
            <select
              value={empleadoId}
        ...
        {empleadoIdInicial && empleadoSel && (
          <div className="mb-3 rounded bg-gray-50 px-2 py-1.5 text-sm text-gray-700">
        ...
        <div className="mb-3 grid grid-cols-2 gap-3">
          ...
          <div>
            <label className="mb-1 block text-xs font-medium text-gray-700">Local</label>
            <select
              value={local}

...
export function EvaluacionesTab() {
  ...
  const stats = useMemo<Stats[]>(() => {
    ...
    return filtrados.map((emp) => {
      ...
      const cronoProgramados = cronoPeriodo.filter(
        (c) => c.publicado && !c.es_franco && c.hora_entrada,
      );
      ...
      const diasFichadosSet = new Set(
        fichadasPeriodo.filter((f) => f.tipo === 'entrada').map((f) => f.fecha),
      );
      ...
      const porcentajePuntual =
        totalPuntualidad > 0 ? Math.round((puntualesTotal / totalPuntualidad) * 100) : 0;
  ...
  const topPuntualidad = [...statsConDatos]
    .sort(
      (a, b) => b.porcentajePuntual - a.porcentajePuntual || b.diasProgramados - a.diasProgramados,
    )
    .slice(0, 10);
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-3 rounded-lg border border-surface-border bg-white p-3">
        ...
        <div className="ml-auto text-xs text-gray-500">
          {statsConDatos.length} empleado{statsConDatos.length !== 1 ? 's' : ''} con datos
          suficientes
        </div>
      </div>

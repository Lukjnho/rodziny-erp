...
export function AnalisisTab() {
  ...
  const rendimientos = useMemo<RendimientoAgg[]>(() => {
    ...
    const agg = new Map<
      string,
      {
        lotes: Array<{ real: number; fecha: string }>;
        tipo: string;
        unidad: 'kg' | 'porciones';
        teorico: number;
      }
        ...
        <header className="flex items-center justify-between border-b border-gray-100 px-4 py-3">
          <div>
            ...
            <p className="text-[11px] text-gray-500">
              Promedio real de los últimos {ventanaDias} días comparado con el rendimiento de la
              receta. Desvío alto sugiere ajustar procesos o actualizar la receta.
            </p>
          </div>
          <span className="text-[10px] text-gray-400">
            {rendimientos.length} receta{rendimientos.length !== 1 ? 's' : ''}
          </span>
        </header>
        {rendimientos.length === 0 ? (
          <div className="px-4 py-6 text-center text-sm text-gray-500">
            Sin datos suficientes en este período. Necesitás recetas con rendimiento teórico y lotes
            de producción con cantidad real.
          </div>
        ) : (
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-50">
                {rendimientos.map((r) => {
                  ...
                  const color =
                    abs < 5 ? 'text-green-700' : abs < 10 ? 'text-amber-700' : 'text-red-700';
                  ...
                  return (
                    <tr key={r.receta_id} className={cn('hover:bg-gray-50', rowBg)}>
                      ...
                      <td className="px-4 py-3 text-xs capitalize text-gray-500">{r.tipo}</td>
                      <td className="px-4 py-3 text-right text-gray-600">{r.lotes}</td>
                      <td className="px-4 py-3 text-right text-gray-600">
                        {fmtNum(r.teorico, r.teoricoUnidad === 'kg' ? 2 : 0)}{' '}
                        <span className="text-[10px] text-gray-400">
                          {r.teoricoUnidad === 'kg' ? 'kg' : 'porc.'}
                        </span>
                      </td>
      ...
      <section className="overflow-hidden rounded-lg border border-surface-border bg-white">
        <header className="flex items-center justify-between border-b border-gray-100 px-4 py-3">
          <div>
            ...
            <p className="text-[11px] text-gray-500">
              Cantidad descartada sobre el total producido en los últimos {ventanaDias} días.
              Motivos más frecuentes a la derecha.
            </p>
          </div>
                  ...
                  const color =
                    m.pctMerma < 3
                      ...
                      : m.pctMerma < 8
                        ...
                        : 'text-red-700';
                  const rowBg =
                    m.pctMerma >= 8 ? 'bg-red-50/50' : m.pctMerma >= 3 ? 'bg-amber-50/30' : '';
                  return (
